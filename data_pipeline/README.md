...
| `build_oakland_boundary.py` | Rebuilds `frontend/public/oakland_boundary.geojson`, the city outline drawn on the map, by dissolving the 137 polygons in `oakland_neighborhoods.geojson` into one land footprint and closing the seams between them. The previous hand-made version left 8 seams as inward spurs and one neighbourhood as a detached piece. Not the legal city limit, which runs out into the Bay. Re-run whenever the neighbourhoods layer changes. Requires `shapely`. |
